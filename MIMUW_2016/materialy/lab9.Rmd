...
Today we are going to play with cancer data.

The instruction below downloads a dataset for 408 patients with breast cancer. 

In the column `outcome` we have information about 3-years survival. 

The goal is to create a score/classifier that is a good prediction for the survival/outcome.

We will do this in two steps.
In the first one we will focus only on two variables: age, ALKBH1. Working on small dataset will make it easier to understand structure of different models.
In the second we will take into account all available variables.
...
1. Use the `lda{MASS}` function to fit the LDA classifier based on two variables age+ALKBH1. 
...
Once we learned structures of various models and saw their decision regions now we are ready to work with all 96 variables.

1. Grow and plot a decision tree based on all variables.
...
Here you have some guides. Use them only if needed.
