...
Je¿eli wektory maj¹ przeciwne zwroty to dane zmienne s¹ ujemnie skorelowane (nie mo¿na jednak oceniæ wartoœci korelacji), je¿eli zwroty s¹ prostopad³e to zmienne s¹ nieskorelowane, a je¿eli zwroty s¹ bliskie to zmienne s¹ dodatnio skorelowane. Ponadto kolorami zosta³ oznaczony na nim podzia³ na klastry metoda kmeans. Z wykresu mo¿emy wyczytaæ, ¿e statystykami które dobrze odró¿niaj¹ grupy s¹ na przyk³ad P13_24_rel, MeanSD_P2, MeanSD_P4, M2_sr, R2_sr, MeanStudMax_M.
...
<li> **Klepsydra** Grupa postawi³a przed sob¹ jasny cel - nie marnowaæ swojego czasu na zadania, których nie potrafi¹ rozwi¹zaæ. Wynika to z najni¿szego œredniego odchylenia jakim charakteryzuj¹ siê cz³onkowie grupy.  </li> 

<li> **Po Pó³**  Grupa kieruj¹ca siê zdrowym rozs¹dkiem, ich motto brzmi - "dla wszystkich po równo", co znajduje odzwierciedlenie w ich strategii, która polega na równomiernym roz³o¿eniu swojego czasu na ka¿d¹ z czêœci. Pozwala to na skupienie siê na zadaniach, które wymagaj¹ wiêcej uwagi. </li>

<li> **YOLO** - Grupa charakteryzuj¹ca siê jedn¹ z najbardziej oczywistych strategii, czyli jej brakiem. Cz³onkowie grupy nie potrafi¹ zorganizowaæ czasu, którym dysponuj¹ aby rozwi¹zaæ test bezstresowo. W pierwszej pozycji testu oraz w trzeciej odznaczaj¹ siê na tle pozosta³ych grup  najd³u¿szym ³¹cznym czasem rozwi¹zywania danej czêœci, natomiast w drugiej i czwartej - najkrótszym. Ponadto ich œrednie maksymalne czasy, oraz odchylenia s¹ stosunkowo du¿e co wskazuje na brak skupienia i poœwiêcanie d³u¿szej chwili na zadanie, które mogliby pomin¹æ i skupiæ siê bardziej na swoich dobrych stronach. </li>
</ul>
...
Poni¿ej znajduje siê tabela przedstawiaj¹ca procentowy udzia³ œrednich ³¹cznych czasów rozwi¹zywania zadañ z ka¿dej pozycji w ka¿dej z grup. Widzimy ¿e grupa **YOLO** spêdzi³a najmniej czasu na czêœci 2 i 4 oraz najwiêcej na 1 i 3. Pamiêtaj¹c, ¿e miêdzy 2, a 3 pozycj¹ odby³a siê przerwa dla uczniów wskazuje to na skupienie siê na pierwszej czêœci kosztem nastêpnej. **Po Pó³** natomiast rozdzieli³o swój czas relatywnie równomiernie pomiêdzy pozycje. Trochê gorzej od Po Pó³ wypad³a **Klepsydra**, równie¿ staraj¹c siê zachowaæ jednolity podzia³ dysponowanym czasem.
...
Widzimy na wykresach, ¿e grupa **Klepsydra** ma najni¿sze œrednie odchylenia przy najni¿szym œrednim maksymalnym czasie rozwiazywania zadania. Oznacza to, ¿e cz³onkowie grupy staraj¹ siê nie spêdziæ du¿o czasu nad ¿adnym zadaniem. Natomiast w  **YOLO** wystêpuj¹ najwiêksze odchylenia standardowe czasów rozwi¹zywania zadañ przy najwy¿szych œrednich maksymalnych czasach rozwi¹zywania zadañ. 
...
tabelka_c[1,3] <- cor.test(wyniki_plus_grupy$MeanScore, wyniki_plus_grupy$`Po Pó³`)$statistic
...
tabelka_c[2,3] <- cor.test(wyniki_plus_grupy$TopPerf, wyniki_plus_grupy$`Po Pó³`)$statistic
...
tabelka_c[3,3] <- cor.test(wyniki_plus_grupy$LowPerf, wyniki_plus_grupy$`Po Pó³`)$statistic
...
Na podstawie analizy danych wyznaczyliœmy 3 ró¿ne grupy szkó³ odpowiadaj¹ce trzem ró¿nym sposob¹ dysponowania czasem podczas pisania testu PISA. Najbardziej efektywn¹ strategia okaza³a siê **Po Pó³**, natomiast niewiele gorsz¹ strategi¹ okaza³a siê **Klepsydra**. Naszym zdaniem efektywnoœæ strategii **Po Pó³** jest wynikiem tego, ¿e nie zmusza studenta do pospiesznego i niedok³adnego rozwi¹zywania zadañ w dalszej czêsci testu, jednoczeœnie daj¹c mu mo¿liwoœæ spêdzenia wiêkszej iloœæi czasu na trudniejszych zadaniach. Strategia **Klepsydra** posiada pierwsz¹ z tych zalet jednak brakuje jej elastycznoœci w organizacji czasu. 